...
//        -NXP\project\Beacon_XBY\app\control\src\speedfast.c
//    Command line =  
//        "D:\Freescale\信标\Beacon_C1
//        -NXP\project\Beacon_XBY\app\control\src\speedfast.c" -D LPLD_K60 -lCN
...
//    List file    =  
//        D:\Freescale\信标\Beacon_C1
//        -NXP\project\Beacon_XBY\iar\FLASH\List\speedfast.s
//
///////////////////////////////////////////////////////////////////////////////
...
// D:\Freescale\信标\Beacon_C1 -NXP\project\Beacon_XBY\app\control\src\speedfast.c
//    1 /*=============================================
//    2     @file         speed.c
//    3     @brief        速度控制
//    4     @programmer   XBY
...
//   82   if(area_temp < 0)
        CMP      R1,#+0
        BPL.N    ??Set_Image_Speed_0
//   83     area_temp = -area_temp;
        RSBS     R1,R1,#+0
//   84   
//   85   if((area_temp >= gi_area_thredhold_1)&&(area_temp <= gi_area_thredhold_2))
??Set_Image_Speed_0:
        LDR.N    R2,??DataTable4_13
        LDRSH    R2,[R2, #+0]
        CMP      R1,R2
        BLT.N    ??Set_Image_Speed_1
        LDR.N    R2,??DataTable4_14
        LDRSH    R2,[R2, #+0]
        CMP      R2,R1
        BLT.N    ??Set_Image_Speed_1
//   86     img_speed_delta = (int)((area_temp - gi_area_thredhold_1) *gf_dece_kp/100);
        ...
        BL       __aeabi_fdiv
        BL       __aeabi_f2iz
        B.N      ??Set_Image_Speed_2
//   87   else if(area_temp > gi_area_thredhold_2)
??Set_Image_Speed_1:
        LDR.N    R2,??DataTable4_14
        LDRSH    R2,[R2, #+0]
        CMP      R2,R1
        BGE.N    ??Set_Image_Speed_3
//   88     img_speed_delta = (int)((area_temp - gi_area_thredhold_1) *gf_dece_kp/80);
        LDR.N    R0,??DataTable4_13
        LDRSH    R0,[R0, #+0]
        SUBS     R0,R1,R0
        BL       __aeabi_i2f
        ...
        LDR      R1,[R1, #+0]
        BL       __aeabi_fmul
        LDR.N    R1,??DataTable4_16  ;; 0x42a00000
        BL       __aeabi_fdiv
        BL       __aeabi_f2iz
        B.N      ??Set_Image_Speed_2
//   89   else if((area_temp < gi_area_thredhold_1)&&(gi_image_center_ave > 10))
??Set_Image_Speed_3:
        LDR.N    R2,??DataTable4_13
        LDRSH    R2,[R2, #+0]
        CMP      R1,R2
        BGE.N    ??Set_Image_Speed_2
        LDR.N    R2,??DataTable4_17
        LDRSH    R2,[R2, #+0]
        CMP      R2,#+11
        BLT.N    ??Set_Image_Speed_2
//   90     img_speed_delta = (int)((area_temp - gi_area_thredhold_1) *gf_acce_kp/100);
        LDR.N    R0,??DataTable4_13
        LDRSH    R0,[R0, #+0]
        SUBS     R0,R1,R0
        BL       __aeabi_i2f
...
//   95 /*===============================================================
//   96 	@brief     弯道减速
//   97 	@function  根据舵机打角程度将速度减小至某一值   
//   98 	@return    改变后的速度值
//   99 	@note      只为初期预判值，后期需精调
//  100 ==================================================================*/
//  101 

        SECTION `.text`:CODE:NOROOT(1)
        THUMB
//  102 int Set_Angle_Speed()
//  103 {   
Set_Angle_Speed:
        PUSH     {R3-R5,LR}
//  104   int32 servo_output;
//  105   int servo_angle;
//  106   int angle_speed_delta;
//  107 //  static float angle_speed_kp = 0.7;//升级为全局变量
//  108   
//  109   servo_output = gl_servo_pwm;
        LDR.N    R0,??DataTable4_19
        LDR      R0,[R0, #+0]
//  110   //servo_angle = Servo_Angle(servo_output);
//  111   servo_angle = gl_direction_set;
        LDR.N    R0,??DataTable4_20
        LDR      R4,[R0, #+0]
//  112   
//  113   if(servo_angle < 0)
        CMP      R4,#+0
        BPL.N    ??Set_Angle_Speed_0
//  114     servo_angle = -servo_angle;
        RSBS     R4,R4,#+0
//  115   
//  116   angle_speed_delta = (int)((gf_angle_speed_kp/1000) * servo_angle);
??Set_Angle_Speed_0:
        LDR.N    R0,??DataTable4_21
        LDR      R0,[R0, #+0]
        LDR.N    R1,??DataTable4_22  ;; 0x447a0000
        BL       __aeabi_fdiv
        MOVS     R5,R0
        MOVS     R0,R4
        BL       __aeabi_i2f
...
//  124 
        ...
        SECTION `.text`:CODE:NOROOT(1)
        THUMB
//  125 void Get_Car_Condition()
//  126 {
Get_Car_Condition:
        PUSH     {R4,LR}
//  127     static int16 si_car_speed = 0;
//  128     
//  129     gi_left_speed = (int16)(Get_Left_Speed());
        BL       Get_Left_Speed
        BL       __aeabi_f2iz
        LDR.N    R1,??DataTable4_23
        STRH     R0,[R1, #+0]
//  130     gi_right_speed = (int16)(Get_Right_Speed());
        BL       Get_Right_Speed
        BL       __aeabi_f2iz
        LDR.N    R1,??DataTable4_24
        STRH     R0,[R1, #+0]
//  131     
//  132     si_car_speed = (int16)((gi_left_speed + gi_right_speed)/2);
        ...
        STRH     R0,[R1, #+0]
//  133     
//  134     gl_car_speed = si_car_speed;
        LDR.N    R0,??DataTable4_25
        LDRSH    R0,[R0, #+0]
        LDR.N    R1,??DataTable4_2
        STR      R0,[R1, #+0]
//  135     
//  136     gl_car_running_distance += (int32)(si_car_speed * 0.5);
        ...
        BL       __aeabi_i2d
        MOVS     R2,#+0
        LDR.N    R3,??DataTable4_27  ;; 0x3fe00000
        BL       __aeabi_dmul
        BL       __aeabi_d2iz
        ADDS     R0,R0,R4
...
//  138 
//  139 /*===============================================================
//  140 	@brief     利用舵机增量计算左右轮差速
//  141 ==================================================================*/
//  142 

        SECTION `.text`:CODE:NOROOT(1)
        THUMB
//  143 void Speed_PWM_Delta(int16 *left_expect_speed, int16 *right_expect_speed, int32 center_expect_speed)
//  144 { 
Speed_PWM_Delta:
        PUSH     {R3-R11,LR}
        MOV      R9,R0
        MOVS     R4,R1
        MOVS     R5,R2
//  145         static int PWM_delta[16] = {0, 8, 15, 23,
//  146                                      28, 35, 39, 44, 
//  147                                      48, 52, 56, 60, 
//  148                                      64, 68, 72, 75};
//  149 
//  150         static float Speed_Inside[16] = {0.9261,0.9161,0.9061,0.8961,
//  151                                           0.8861,0.8761,0.8661,0.8561,
//  152                                           0.8411,0.8311,0.8211,0.8111,
//  153                                           0.8011,0.7861,0.7711,0.7561};
//  154 	static float Speed_Outside[16] ={1.0739,1.0839,1.0939,1.1039,
//  155                                           1.1139,1.1239,1.1339,1.1439,
//  156                                           1.1589,1.1689,1.1789,1.1889,
//  157                                           1.1989,1.2139,1.2289,1.2439};
//  158 
//  159 	int flag = 0, i = 0;
        MOVS     R6,#+0
        MOVS     R1,#+0
//  160 	int16  speed_left_temp = 0, speed_right_temp = 0;
        MOVS     R7,#+0
        MOVS     R8,#+0
//  161         int16 servo_PWM_delta = 0; 
        MOVS     R0,#+0
//  162         servo_PWM_delta = Get_Servo_PWM_Delta();
        BL       Get_Servo_PWM_Delta
//  163         
//  164         if (center_expect_speed == 0)
        CMP      R5,#+0
        BNE.N    ??Speed_PWM_Delta_0
//  165         {
//  166    		speed_left_temp = 0;
        MOVS     R7,#+0
//  167 		speed_right_temp = 0;     
        MOVS     R8,#+0
        B.N      ??Speed_PWM_Delta_1
//  168         }
//  169         else if (servo_PWM_delta >= -10 && servo_PWM_delta <= 10)
??Speed_PWM_Delta_0:
...
//  170         {
//  171             speed_left_temp = center_expect_speed;
        MOVS     R7,R5
//  172             speed_right_temp = center_expect_speed;  
        MOV      R8,R5
        B.N      ??Speed_PWM_Delta_1
//  173         }
//  174         else if (servo_PWM_delta < -10)//向右转
??Speed_PWM_Delta_2:
        SXTH     R0,R0            ;; SignExt  R0,R0,#+16,#+16
        CMN      R0,#+10
        BGE.N    ??Speed_PWM_Delta_3
//  175         {
//  176              servo_PWM_delta = -servo_PWM_delta;
        SXTH     R0,R0            ;; SignExt  R0,R0,#+16,#+16
        RSBS     R0,R0,#+0
//  177              for(i = 0; i <= 14; i++)
        MOVS     R1,#+0
...
//  178              {
//  179                 if (servo_PWM_delta > PWM_delta[i] && servo_PWM_delta <= PWM_delta[i+1])
        LDR.N    R2,??DataTable4_28
        LDR      R2,[R2, R1, LSL #+2]
        SXTH     R0,R0            ;; SignExt  R0,R0,#+16,#+16
        CMP      R2,R0
...
//  189             }
//  190              
//  191             speed_left_temp = (int16)(center_expect_speed * (Speed_Outside[flag]+0.035));
??Speed_PWM_Delta_7:
        MOVS     R0,R5
        BL       __aeabi_i2d
        MOV      R10,R0
        ...
        LDR      R0,[R0, R6, LSL #+2]
        BL       __aeabi_f2d
        LDR.N    R2,??DataTable4_30  ;; 0x1eb851ec
        LDR.N    R3,??DataTable4_31  ;; 0x3fa1eb85
        BL       __aeabi_dadd
        MOV      R2,R10
        MOV      R3,R11
        BL       __aeabi_dmul
        BL       __aeabi_d2iz
        MOVS     R7,R0
//  192             speed_right_temp = (int16)(center_expect_speed *(Speed_Inside[flag]-0.012));
        MOVS     R0,R5
        BL       __aeabi_i2d
        MOV      R10,R0
        MOV      R11,R1
        ...
        LDR      R0,[R0, R6, LSL #+2]
        BL       __aeabi_f2d
        LDR.N    R2,??DataTable4_33  ;; 0xbc6a7efa
        LDR.N    R3,??DataTable4_34  ;; 0xbf889374
        BL       __aeabi_dadd
        MOV      R2,R10
        MOV      R3,R11
        BL       __aeabi_dmul
...
//  197              {
//  198                 if (servo_PWM_delta > PWM_delta[i] && servo_PWM_delta <= PWM_delta[i+1])
        LDR.N    R2,??DataTable4_28
        LDR      R2,[R2, R1, LSL #+2]
        SXTH     R0,R0            ;; SignExt  R0,R0,#+16,#+16
        CMP      R2,R0
...
//  208             }
//  209 
//  210             speed_right_temp = (int16)(center_expect_speed * (Speed_Outside[flag]+0.035));
??Speed_PWM_Delta_11:
        MOVS     R0,R5
        BL       __aeabi_i2d
        MOV      R10,R0
        ...
        BL       __aeabi_d2iz
        MOV      R8,R0
//  211             speed_left_temp = (int16)(center_expect_speed *(Speed_Inside[flag]-0.012));
        MOVS     R0,R5
        BL       __aeabi_i2d
        MOV      R10,R0
        MOV      R11,R1
...
//  212         }
//  213 
//  214 	*left_expect_speed = speed_left_temp;
??Speed_PWM_Delta_1:
        STRH     R7,[R9, #+0]
//  215 	*right_expect_speed = speed_right_temp;
        STRH     R8,[R4, #+0]
//  216 }
